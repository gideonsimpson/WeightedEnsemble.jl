...
n_bins = length(B₀);
K̃ = WeightedEnsemble.tbuild_coarse_transition_matrix(mutation!, bin_id, x0_vals,bin0_vals, n_bins, n_samples_per_bin);
...
B = deepcopy(B₀);
Random.seed!(200)
trun_we!(E, B, mutation!, selection!, rebin!, n_we_steps);
p_est = f.(E.ξ) ⋅ E.ω
